#!/usr/bin/env python
"""
train_hierarchical_continue.py — 分层强化学习 PPO **续训**脚本

基于 train_hierarchical.py，用于从已有 checkpoint 继续训练。
与原始脚本相比有以下关键修改:

  ★ 1. 续训时不加载 optimizer 状态
       - Adam 的一阶/二阶动量是在旧 LR=1e-3 下积累的，
         直接加载会导致新 LR=3e-4 的更新步长不正确。
       - 让 Adam 以新 LR 重新适配，使梯度更新立即生效。

  ★ 2. 续训时手动重置 Q2 的 log_std
       - 旧训练中 log_std 漂移到 ~0.2 (σ≈1.2)，探索噪声过大。
       - 重置为 OFFSET_LOG_STD_INIT=-0.5 (σ≈0.607)，
         让策略在合理探索范围内继续优化。

  ★ 3. 训练循环中加入学习率线性衰减
       - LR 从 LEARNING_RATE 线性衰减至 10% (防止后期震荡)。
       - 公式: lr = LEARNING_RATE × max(1 - iter/total_iters, 0.1)
       - 同时记录到 TensorBoard (train/learning_rate)。
...
使用方式:
  ...
  # 从 checkpoint 续训:
  python train_hierarchical_continue.py --resume checkpoints/policy_iter1450.pth
  # 也可从头训练 (行为与原始脚本一致):
  python train_hierarchical_continue.py
"""
...
def train(resume_path=None):
    ...
    if resume_path and os.path.exists(resume_path):
        ...
        # ★ 不加载 optimizer 状态，让 Adam 用新 LR 重新适配
        # if 'optimizer_state_dict' in ckpt:
        #     trainer.optimizer.load_state_dict(ckpt['optimizer_state_dict'])
        # ★ 重置 Q2 的 log_std 到初始值 (消除漂移)
        with torch.no_grad():
            policy.q2.log_std.fill_(cfg.OFFSET_LOG_STD_INIT)
        print(f"  Reset q2.log_std to {cfg.OFFSET_LOG_STD_INIT} (sigma={np.exp(cfg.OFFSET_LOG_STD_INIT):.3f})")
        # ★ 重置 Q2 输出层 (打破卡死的负 mean)
        # 只重置最后一层 Linear(32,1)，保留 LSTM + 第一层 FC 学到的特征
        with torch.no_grad():
            nn.init.orthogonal_(policy.q2.fc_mean[-1].weight, gain=0.01)
            policy.q2.fc_mean[-1].bias.fill_(0.0)
        print(f"  Reset q2.fc_mean output layer (mean ≈ 0)")
    ...
    for iteration in range(start_iteration, num_iterations):
        ...
        for step in range(rollout_steps):
            ...
            # 从策略分布中采样动作
            # select_action 内部已做 clamp(±OFFSET_RANGE) 并算对应的 log_prob
            action_info = policy.select_action(state_tensor)
            goal = action_info['goal']
            p_off = action_info['offset']  # 已 clamp，无需再 clip

            # 环境交互
            next_obs, reward, done, info = env.step([goal, p_off])

            # 存储到 rollout buffer
            # offset 和 log_prob_offset 都对应环境实际执行的动作 (已 clamp)
        ...
        #lr_now = cfg.LEARNING_RATE * max(frac, 0.1)  # 最低不低于 LR 的 10%
